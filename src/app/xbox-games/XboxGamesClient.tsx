...
interface FullAccount extends XboxGame {
  games: string[];
}

export default function XboxGamesClient() {
  const [gamesSubTab, setGamesSubTab] = useState<"buyOnAccount" | "fullAccounts">("buyOnAccount");
  const addItem = useCartStore((s) => s.addItem);
  const paymentMethod = useCartStore((s) => s.paymentMethod);
  const [addedItems, setAddedItems] = useState<Set<string>>(new Set());

  const xboxGames = xboxGamesData as { buyOnAccount: XboxGame[]; fullAccounts: FullAccount[] };

  const handleAddToCart = (game: XboxGame) => {
    addItem({
      id: game.id,
      name: game.title,
      region: game.titleAr,
      value: gamesSubTab === "buyOnAccount" ? "شراء في حسابك" : "حساب كامل",
      quantity: 1,
      ...
      <div className="flex justify-center mb-8">
        <div className="flex gap-2 p-1.5 bg-navy-dark/80 backdrop-blur-xl rounded-full border border-white/[0.06] shadow-premium w-full max-w-md">
          <button
            ...
            className={`flex-1 py-3 px-4 rounded-full text-sm font-black transition-all duration-300 ease-premium ${
